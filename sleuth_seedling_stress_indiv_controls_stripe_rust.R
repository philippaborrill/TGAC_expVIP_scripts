...
samples_to_use <- "SRP041017_stripe_rust"
...
s2c <- read.table ("Y:\\expression_browser\\TGAC_assembly\\analysis\\seedling_stress_analysis\\SRP041017_stripe_rust.txt",sep="\t", header = TRUE, stringsAsFactors=FALSE)
s2c
...
so <- sleuth_test(so, which_beta = 'conditionstripe rust pathogen CYR31 24 hours')
...
results_table <- sleuth_results(so, 'conditionstripe rust pathogen CYR31 24 hours') 
head(results_table)
write.csv(results_table, "stripe rust pathogen CYR31 24 hours_sorted_abundances.csv")
...
write.csv(edited_results_table, "stripe rust pathogen CYR31 24 hours_sorted_abundances_q_value_under_0.05.csv")


# perform differential analysis for each condition vs none
so <- sleuth_test(so, which_beta = 'conditionstripe rust pathogen CYR31 48 hours')

# save results tables

results_table <- sleuth_results(so, 'conditionstripe rust pathogen CYR31 48 hours') 
head(results_table)
write.csv(results_table, "stripe rust pathogen CYR31 48 hours_sorted_abundances.csv")

edited_results_table <- results_table[results_table$qval<0.05,]
edited_results_table <- (edited_results_table[complete.cases(edited_results_table),])
dim(edited_results_table)
write.csv(edited_results_table, "stripe rust pathogen CYR31 48 hours_sorted_abundances_q_value_under_0.05.csv")


# perform differential analysis for each condition vs none
so <- sleuth_test(so, which_beta = 'conditionstripe rust pathogen CYR31 72 hours')

# save results tables

results_table <- sleuth_results(so, 'conditionstripe rust pathogen CYR31 72 hours') 
head(results_table)
write.csv(results_table, "stripe rust pathogen CYR31 72 hours_sorted_abundances.csv")

edited_results_table <- results_table[results_table$qval<0.05,]
edited_results_table <- (edited_results_table[complete.cases(edited_results_table),])
dim(edited_results_table)
write.csv(edited_results_table, "stripe rust pathogen CYR31 72 hours_sorted_abundances_q_value_under_0.05.csv")

# to save so

saveRDS(so, file = 'condition SRP041017_stripe_rust.rds')
