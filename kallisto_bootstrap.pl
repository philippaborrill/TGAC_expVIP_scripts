...
my $tgac_path_puccinia = '/tgac/public/reads/puccinia_striiformis/';
...
my $output_dir = "$path/expression_browser/TGAC_assembly/analysis/kallisto_results_bootstrap/";
...
my $triticum_TGAC_single_list = 'column_triticum_TGAC_single_dir_names.txt';
my $puccinia_TGAC_paired_list = 'column_puccinia_TGAC_paired_dir_names.txt';
...
#use this line if paired end reads:
	print SLURM "kallisto quant -i $index -o $output_dir/$prefix -b 100 --pseudobam -t 8 $prefix"."_1.fastq.gz $prefix"."_2.fastq.gz | samtools view -Sb - > $output_dir/bam/$prefix.bam \n";
...
### second do $puccinia_TGAC_paired_list #note these files are not gzipped
#open the input file and go through the lines one by one so go to each directories where the fastq.gz should be located
chdir("$output_dir") or die "couldn't move to output directory";

#mkdir "fastqc" or  die "couldn't make directory fastqc in $output_dir\n";
open (INPUT_FILE, "$puccinia_TGAC_paired_list") || die "couldn't open the input file $puccinia_TGAC_paired_list!";
		    while (my $dir = <INPUT_FILE>) {
			chomp $dir;
...
chdir("$tgac_path_puccinia/$dir") or die "couldn't move to tgac specific read directory";
...
@files = grep(/([\w\+\-]+)_1.fastq/,readdir(DIR));
closedir(DIR);
...
#  ($prefix) = $file =~ /([\w\+\-]+).fastq.gz/;
 ...
 my $tmp_file = "$output_dir/tmp/kallisto_paired_puccinia_SLURM.$prefix";

  open (SLURM, ">$tmp_file") or die "Couldn't open temp file\n";
  my $SLURM_header = $SLURM_header;
  $SLURM_header =~ s/JOBNAME/$prefix/g;
  print SLURM "$SLURM_header\n\n";
  print SLURM "\ncd $tgac_path_puccinia/$dir\n";

  print SLURM "source kallisto-0.42.3\n";
  print SLURM "source samtools-0.1.19\n";

#use this line if paired end reads:
	print SLURM "kallisto quant -i $index -o $output_dir/$prefix -b 100 --pseudobam -t 8 $prefix"."_1.fastq $prefix"."_2.fastq | samtools view -Sb - > $output_dir/bam/$prefix.bam \n";
...
### third do $triticum_expr_browser_paired_list
#open the input file and go through the lines one by one so go to each directories where the fastq.gz should be located
chdir("$output_dir") or die "couldn't move to output directory";
...
### fourth do $triticum_TGAC_single_list
#open the input file and go through the lines one by one so go to each directories where the fastq.gz should be located
chdir("$output_dir") or die "couldn't move to output directory";

#mkdir "fastqc" or  die "couldn't make directory fastqc in $output_dir\n";
open (INPUT_FILE, "$triticum_TGAC_single_list") || die "couldn't open the input file $triticum_TGAC_single_list!";
		    while (my $dir = <INPUT_FILE>) {
			chomp $dir;
...
@files = grep(/([\w\+\-]+).fastq.gz/,readdir(DIR));
closedir(DIR);
...
#use this line if single end reads:
 ...
 my $tmp_file = "$output_dir/tmp/kallisto_single_triticum_SLURM.$prefix";
...
#use this line if paired end reads:
	print SLURM "kallisto quant -i $index -o $output_dir/$prefix -b 100 --pseudobam -t 8 --single -l 150 -s 50 $prefix".".fastq.gz  | samtools view -Sb - > $output_dir/bam/$prefix.bam\n";
